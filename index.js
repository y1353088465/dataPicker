let province = ["北京市", "天津市", "河北省", "河南省", "山西省", "山东省", "内蒙古自治区", "辽宁省", "吉林省", "黑龙江省", "上海市", "江苏省", "浙江省", "福建省", "江西省", "安徽省", "湖北省", "湖南省", "广东省", "广西壮族自治区", "海南省", "重庆市", "四川省", "贵州省", "云南省", "西藏自治区", "陕西省", "甘肃省", "青海省", "宁夏回族自治区", "新疆维吾尔自治区", "台湾省", "香港特别行政区", "澳门特别行政区"];
let city = [
  ["市辖区"],
  ["市辖区", "市辖县"],
...
class picker {
    constructor({el,showDom,success}){
      // 默认选中地区数组下标初始化
      this.p_index = 0;
      this.c_index = 0;
      this.a_index = 0;
      ...
      this.showDom = showDom
        ...
        this.el.innerHTML = `
        <div class="picker_mask_"></div>
        <div class="data_list_box">
            <p class="event_btns"><span class="cancel_btn">取消</span><span class="sure_btn">确定</span></p>
            <ul class="data_list">
        ...
        `
        this.eachProvince();
        this.eachCity(this.p_index);
        this.eachArea(this.p_index,this.c_index);  
        // this.sure()       
        console.log(success) 
        this.sure(success);
        this.cancel();
    ...
    eachProvince(){
      // provice数据为一维数组
        let provinceDom = document.getElementsByClassName("province")[0];        
        province.forEach((v,k)=>{
          let newP = document.createElement("p");
          newP.className = "p_eve";
          newP.innerHTML = v;
          newP.index = k;
          provinceDom.appendChild(newP)
        })
        this.pClick()
        provinceDom.children[this.p_index].className = "p_eve picker_choosed_active";
    ...
    eachCity(index){
        // city数据为2维数组
        let c_str = '';
        city[index].forEach((v,k)=>{
            c_str += `<p class="c_eve" index=${k}>${v}</p>`
        })
        let cityDom = document.getElementsByClassName("city")[0];
        cityDom.innerHTML = c_str;
        this.cClick()        
        cityDom.children[0].className = "c_eve picker_choosed_active";
    }
    //初始化区循环出数据
    eachArea(index1,index2){
        // area数据为3维数组
        let a_str = '';
        area[index1][index2].forEach((v,k)=>{
          a_str += `<p class="a_eve" index=${k}>${v}</p>`
        })
        let areaDom = document.getElementsByClassName("area")[0];
        areaDom.innerHTML = a_str;
        this.aClick();
        areaDom.children[this.a_index].className = "a_eve picker_choosed_active";
    }
    //省点击
    pClick(){  
      let provinceDom = document.getElementsByClassName("province")[0];          
        provinceDom.onclick = function(ev){
          this.a_index = 0;
          this.c_index = 0;
          let e = ev || window.event;
          if(e.target.nodeName == "P"){
            for (let i = 0;i < provinceDom.children.length; i++){
              provinceDom.children[i].className = "p_eve";
            }
            //记录当前选中的省份下标
            this.p_index = e.target.index;
  
            this.eachCity(this.p_index)
            this.eachArea(this.p_index,this.c_index)          
            let pEveDom = document.getElementsByClassName("p_eve")[e.target.index];
            pEveDom.className = "p_eve picker_choosed_active";
          }
        }.bind(this);  
    }
    //市点击
    cClick(){  
      let cityDom = document.getElementsByClassName("city")[0];      
      cityDom.onclick = function(ev){
          this.a_index = 0;        
          let e = ev || window.event;
          if(e.target.nodeName == "P"){
            for (let i = 0;i < cityDom.children.length; i++){
              cityDom.children[i].className = "c_eve";
            }
            //记录当前选中的城市下标
            this.c_index = Array(e.target.getAttribute("index"));
            this.eachArea(this.p_index,this.c_index)
            let cEveDom = document.getElementsByClassName("c_eve")[this.c_index];
            cEveDom.className = "c_eve picker_choosed_active";
          }
        }.bind(this);
    }
    //区点击
    aClick(){
      let areaDom = document.getElementsByClassName("area")[0];
      areaDom.onclick = function(ev){   
        let e = ev || window.event;
        if(e.target.nodeName == "P"){
          for (let i = 0;i < areaDom.children.length; i++){
            areaDom.children[i].className = "a_eve";
          }
          //记录当前选中的城市下标
          this.a_index = Array(e.target.getAttribute("index"));
          this.eachArea(this.p_index,this.c_index)
          let aEveDom = document.getElementsByClassName("a_eve")[this.a_index];
          aEveDom.className = "c_eve picker_choosed_active";
        }
      }.bind(this);
    }
    ...
    cancel(){
      let sbDom = document.getElementsByClassName("cancel_btn")[0];   
      sbDom.onclick = function (){
        console.log()
        document.getElementsByClassName("picker_mask_")[0].style.display = "none";
        document.getElementsByClassName("data_list_box")[0].style.display = "none";
